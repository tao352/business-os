...
export function generatePaymentSchedule(rawInput: PaymentPlanInput): PaymentScheduleResult {
  const input = PaymentPlanInputSchema.parse(rawInput);

  const totalPrice = roundTwoDecimals(input.totalPrice);
  const downPaymentAmount = roundTwoDecimals(totalPrice * (input.downPaymentPercent / 100));
  const deliveryPercent = input.deliveryPaymentPercent ?? 0;
  const deliveryAmount = roundTwoDecimals(totalPrice * (deliveryPercent / 100));
  
  const remainingForInstallments = roundTwoDecimals(totalPrice - downPaymentAmount - deliveryAmount);

  const freqMonths = getFrequencyMonths(input.frequency);
  const installmentsPerYear = 12 / freqMonths;
  const installmentsCount = Math.round(input.installmentsYears * installmentsPerYear);

  if (installmentsCount <= 0) {
    throw new Error('Installments count must be greater than zero');
  }

  const baseInstallmentAmount = roundTwoDecimals(remainingForInstallments / installmentsCount);
  ...
  if (deliveryAmount > 0) {
    const deliveryDueDate = input.deliveryDate ?? addMonthsToDate(input.startDate, installmentsCount * freqMonths);
    schedule.push({
      installmentNumber: itemCounter++,
  ...
  const maintenanceAmount = roundTwoDecimals(totalPrice * (maintenancePercent / 100));
  if (maintenanceAmount > 0) {
    const maintenanceDueDate = input.deliveryDate ?? addMonthsToDate(input.startDate, installmentsCount * freqMonths);
    schedule.push({
      installmentNumber: itemCounter++,
