import type { FilterAST, FilterCondition, FilterGroup, SortConfig } from '@business-os/types';

export class InvalidQueryFieldError extends Error {
  constructor(public readonly field: string, public readonly entityType: string) {
    super(`Invalid or disallowed query field '${field}' for entity '${entityType}'`);
...
function compileCondition(
  ...
  const colExpr = resolveColumnExpression(entityType, cond.field, cond.is_custom, cond.value);
  ...
  switch (cond.operator) {
    ...
    case 'NOT_IN': {
      ...
      params.push(arr);
      return { sql: `${colExpr} != ALL($${curIndex})`, nextIndex: curIndex + 1 };
    }
    ...
    case 'BETWEEN': {
      if (!Array.isArray(cond.value) || cond.value.length !== 2) {
        throw new Error(`Operator BETWEEN requires an array of 2 values for field '${cond.field}'`);
      }
...
export function compileFilterGroup(
  ...
  for (const item of group.conditions) {
    if ('logical' in item) {
      const res = compileFilterGroup(entityType, item as FilterGroup, curIndex, params);
      if (res.sql && res.sql !== 'TRUE') {
        parts.push(`(${res.sql})`);
        curIndex = res.nextIndex;
      }
    } else {
      const res = compileCondition(entityType, item as FilterCondition, curIndex, params);
      parts.push(res.sql);
      curIndex = res.nextIndex;
    }
  }
...
export function compileSortToSql(entityType: string, sortConfig?: SortConfig[]): string {
  ...
  for (const sort of sortConfig) {
    const colExpr = resolveColumnExpression(entityType, sort.field, sort.is_custom);
