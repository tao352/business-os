  ...
  describe('Dynamic Zod Compiler Unit Tests', () => {
    const mockDefs: CustomFieldDefinition[] = [
      ...
      },
      {
        ...
        validation_rules: {
          options: ['Core & Shell', 'Semi-Finished', 'Fully Finished', 'Ultra Super Lux'],
        },
        is_required: false,
        display_order: 2,
        is_active: true,
    ...
    it('rejects numbers outside specified min/max bounds', () => {
      ...
      expect(() => validateCustomData(mockDefs, tooLow)).toThrow(CustomFieldValidationError);

      const tooHigh = { budget: 999999999 };
      expect(() => validateCustomData(mockDefs, tooHigh)).toThrow(CustomFieldValidationError);
    });
  ...
  describe('Live Custom Fields Service & CRM Integration', () => {
    ...
    it('ensures custom field definitions are strictly isolated between tenants', async () => {
      ...
      const alienLead = await createLead(alienContext, {
        ...
        phone: '+201000112233',
        customData: {
          some_random_field: 'Allowed because Alien org has no required budget field',
        },
      });
