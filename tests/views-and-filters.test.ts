...
import {
  registerUser,
  createOrganization,
} from "../packages/core/src/auth/auth-service.js";
...
describe("Phase 6: Views, Virtualized Tables, Safe AST Filters & Search", () => {
  describe("Unit: Filter AST & Query Compiler Security", () => {
    ...
    it("should compile custom fields with numeric cast when value is a number", () => {
      ...
      const res = compileFilterGroup("leads", group, 1, params);
      expect(res.sql).toBe(
        "NULLIF(custom_data->>'budget', '')::numeric >= $1 AND (custom_data->>'district') = $2",
      );
    ...
    it("should strictly reject malicious field names (SQL injection defense)", () => {
      ...
      const maliciousGroup: FilterGroup = {
        ...
        conditions: [
          {
            field: "full_name; DROP TABLE leads;--",
            operator: "EQUALS",
            value: "bad",
          },
        ],
      ...
      expect(() =>
        compileFilterGroup("leads", maliciousGroup, 1, params),
      ).toThrow(InvalidQueryFieldError);
    });
    ...
    it("should strictly reject invalid operators", () => {
      ...
      expect(() =>
        compileFilterGroup("leads", invalidGroup, 1, params),
      ).toThrow(InvalidQueryOperatorError);
    });
    ...
    it("should compile sort configurations safely", () => {
      ...
      expect(sortSql).toBe(
        'ORDER BY "status" ASC, "created_at" DESC NULLS LAST, (custom_data->>\'budget\') DESC',
      );
    });
    ...
    it("should compile entity search queries", () => {
      ...
      expect(res.sql).toBe(
        '("full_name" ILIKE $1 OR "phone" ILIKE $1 OR "email" ILIKE $1)',
      );
  ...
  describe("Integration: Live Database Querying, Filtering, Views & Tenant Isolation", () => {
    ...
    it("should search leads by full text term", async () => {
      const nameSearch = await queryEntities(orgAContext, "leads", {
        search: "Mona",
      });
      expect(nameSearch.total).toBe(1);
      expect((nameSearch.data[0] as any).full_name).toBe("Mona Zaki");

      const phoneSearch = await queryEntities(orgAContext, "leads", {
        search: "0103333",
      });
    ...
    it("should enforce Salesperson row-level isolation automatically during queries", async () => {
      ...
      expect(namesA).toEqual(
        expect.arrayContaining(["Ahmed El-Masry", "Mona Zaki"]),
      );
      ...
      expect(namesB).toEqual(
        expect.arrayContaining(["Karim Abdelaziz", "Tamer Hosny"]),
      );
    ...
    it("should create, retrieve, execute, update and delete a Saved View", async () => {
      ...
      await expect(getSavedView(orgAContext, savedView.id)).rejects.toThrow(
        ViewNotFoundError,
      );
    });
    ...
    it("should enforce private vs shared view permissions within the same tenant", async () => {
      ...
      await expect(
        getSavedView(salespersonBContext, privateView.id),
      ).rejects.toThrow(ViewForbiddenError);

      // Salesperson B attempting to delete private view throws ViewForbiddenError
      await expect(
        deleteSavedView(salespersonBContext, privateView.id),
      ).rejects.toThrow(ViewForbiddenError);
    });
  });
});
