...
describe('Phase 8: Robust Import & Export Engine (Excel / CSV)', () => {
  describe('Unit: RFC 4180 CSV Parsing & Generation', () => {
    ...
    it('should strip UTF-8 BOM when parsing and inject UTF-8 BOM when generating', () => {
      ...
      const generated = generateCsv(['اسم العميل', 'الهاتف'], [{ 'اسم العميل': 'محمود', الهاتف: '0105555' }]);
    ...
    it('should auto-detect multi-lingual Arabic and English headers', () => {
      const arabicLeadHeaders = ['اسم العميل', 'رقم الهاتف', 'البريد الالكتروني', 'الميزانية'];
  ...
  describe('Integration: Pre-Flight Dry Run, Ingestion & Filtered Export', () => {
    ...
    it('should execute pre-flight dry-run validation with line-by-line error reporting', async () => {
      const csvData = `اسم العميل,رقم الهاتف,الميزانية\n` +
      ...
      const dryRun = await validateAndDryRunImport(orgAContext, 'leads', csvData);
      ...
      expect(dryRun.errors.some((e) => e.rowNumber === 3 && e.field === 'phone')).toBe(true);
      expect(dryRun.errors.some((e) => e.rowNumber === 4 && e.field === 'full_name')).toBe(true);
      expect(dryRun.errors.some((e) => e.rowNumber === 5 && e.field === 'custom_data')).toBe(true);
      expect(dryRun.errors.some((e) => e.rowNumber === 6 && e.field === 'phone')).toBe(true);
    ...
    it('should batch import leads and handle duplicate strategies (SKIP vs UPDATE)', async () => {
      const batchCsv = `Full Name,Phone,Email,budget\n` +
      ...
      const duplicateCsv = `Full Name,Phone,Email,budget\n` +
      ...
      const leadsAfterSkip = await listLeads(orgAContext, { search: '01077778888' });
      ...
      const leadsAfterUpdate = await listLeads(orgAContext, { search: '01077778888' });
    ...
    it('should batch import real estate units and update project total units', async () => {
      const unitsCsv = `Unit #,Type,Area,Price\n` +
      ...
      expect(units.map((u) => u.unit_number)).toEqual(expect.arrayContaining(['U-201', 'U-202', 'U-203']));
    });
